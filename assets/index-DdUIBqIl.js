...
`).map(s=>s.replace(/^[-•*\d.)\s]+/,"").trim()).filter(s=>s.length>2&&s.length<120).slice(0,3);if(t.length===0)throw new Error("No completions returned from the model.");return t}async function re(e,n){const a=n?`
...
In 2-3 sentences, explain in plain English what the director likely meant and what specific directorial concern they were addressing. Be concrete and practical. [/INST]`;return(await B(t)).trim()}function ie(e,n){const a=m.currentSession,t=m.parsedScript||[];let s=1;e.innerHTML=`
  ...
  `;const o=e.querySelector("#script-content"),i=e.querySelector("#page-jump"),r=e.querySelector("#note-page");function c(h){if(t.length===0)return;const f=Math.max(1,Math.min(t.length,h));s=f,i.value=f,r.value=f,o.innerHTML=I(t,f)}e.querySelector("#prev-page").addEventListener("click",()=>c(s-1)),e.querySelector("#next-page").addEventListener("click",()=>c(s+1)),i.addEventListener("change",()=>c(parseInt(i.value)||1)),r.addEventListener("change",()=>{const h=parseInt(r.value)||1;t.length>0?c(h):s=h}),e.querySelector("#btn-end").addEventListener("click",()=>n("#end-session"));const l=e.querySelector("#note-content"),d=e.querySelector("#suggestions-area"),g=e.querySelector("#suggestions-list"),u=e.querySelector("#note-error"),p=e.querySelector("#note-success"),b=e.querySelector("#btn-suggest"),x=e.querySelector("#btn-save"),J=e.querySelector("#notes-count");b.addEventListener("click",async()=>{const h=l.value.trim();if(!h){E("Write some of the note first before requesting suggestions.");return}k(),b.disabled=!0,b.textContent="Thinking…",d.classList.add("hidden");try{const f=await oe(h);g.innerHTML=f.map(y=>`<button class="suggestion-chip">${_(y)}</button>`).join(""),d.classList.remove("hidden"),g.querySelectorAll(".suggestion-chip").forEach(y=>{y.addEventListener("click",()=>{const S=l.value.trimEnd(),L=y.textContent.trimStart(),$=(S.endsWith(".")||S.endsWith("?")||S.endsWith("!")," ");l.value=S+$+L,d.classList.add("hidden"),l.focus()})})}catch(f){E(`Suggestion failed: ${f.message}`)}finally{b.disabled=!1,b.textContent="Suggest Completion"}}),x.addEventListener("click",async()=>{const h=l.value.trim();if(!h){E("Note content cannot be empty.");return}k(),x.disabled=!0,x.textContent="Saving…";const f=parseInt(r.value)||s,y=le(t,f),S={content:h,page_number:f,scene:e.querySelector("#note-scene").value.trim()||null,actor:e.querySelector("#note-actor").value.trim()||null,emotional_category:e.querySelector("#note-cat").value||null,line_snippet:y,timestamp_seconds:Math.floor(Date.now()/1e3)};try{const L=await V(a.id,S);m.sessionNotes.push(L);const $=m.sessionNotes.length;J.textContent=`${$} note${$!==1?"s":""}`,l.value="",e.querySelector("#note-scene").value="",e.querySelector("#note-actor").value="",e.querySelector("#note-cat").value="",d.classList.add("hidden"),p.textContent="Note saved!",p.classList.remove("hidden"),setTimeout(()=>p.classList.add("hidden"),2500),l.focus()}catch(L){E(`Failed to save: ${L.message}`)}finally{x.disabled=!1,x.textContent="Complete Note"}});function E(h){u.textContent=h,u.classList.remove("hidden"),p.classList.add("hidden")}function k(){u.classList.add("hidden"),p.classList.add("hidden")}}function I(e,n){const a=e.find(t=>t.pageNumber===n);return a?`
    <div class="script-page">
      <div class="script-page-label">Page ${n}</div>
      <pre class="script-text">${_(a.text||"(no text on this page)")}</pre>
    </div>
  `:`<p class="script-missing">Page ${n} not available.</p>`}function ce(){return`
    <div class="no-script-notice">
      <p>No script uploaded.</p>
      <p class="muted">Page numbers are set manually in the note form.</p>
    </div>
  `}function le(e,n){const a=e.find(t=>t.pageNumber===n);return!a||!a.text?null:a.text.substring(0,200)}function _(e){return e?String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"):""}async function de(e,n){const a=m.currentSession;e.innerHTML=`
    <div class="page-layout">
      <header class="topbar">
        <button class="btn btn-ghost btn-sm" id="btn-back">← Dashboard</button>
        <div class="topbar-center">
          <span class="logo-mark">▶</span>
          <span class="topbar-title">${N(a.productionName)}</span>
        </div>
  ...
  `}function N(e){return e?String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"):""}function U(e){return e.charAt(0).toUpperCase()+e.slice(1)}function me(e,n){const a=j();e.innerHTML=`
    <div class="page-layout">
      <header class="topbar">
        <button class="btn btn-ghost btn-sm" id="btn-back">← Back</button>

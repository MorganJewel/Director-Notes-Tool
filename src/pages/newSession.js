...
export function renderNewSession(container, navigate) {
  ...
  async function handleFile(file) {
    ...
    uploadZone.querySelector('label span:nth-child(2)').textContent = file.name
...
async function parsePDF(file, onStatus) {
  ...
  if (!lib) throw new Error('PDF.js is not loaded — check your internet connection and reload.')
  ...
  if (avgChars < 50) {
    onStatus('No text found — this looks like a scanned PDF. Loading OCR…', 'loading')
    await loadTesseract()
    const ocrCount = Math.min(10, pdf.numPages)
    const ocrPages = await ocrPDF(buffer, ocrCount, onStatus)
    // Replace first N pages with OCR results; keep the rest as-is (likely empty)
...
function loadTesseract() {
  return new Promise((resolve, reject) => {
    ...
    s.onload = resolve
    s.onerror = () => reject(new Error('Failed to load Tesseract.js. Check your connection.'))
